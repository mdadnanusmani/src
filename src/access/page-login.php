...
if(isset($_POST['emp_id']) && isset($_POST['emp_password']))
{
        ...
        $sqql=" INSERT INTO `users_log`( `username`, `time`, `action`, `ip`) VALUES ('".$row['username']."','".date('d/m/Y h:i:sa')."','Login','".$_SERVER['REMOTE_ADDR']."')";
        	$qqq=mysqli_query($connn,$sqql) ;
                    if($_SESSION['admin_ok']=='1')
        			    echo '<script>window.location = "page-otp.php";</script>';
        			else
        			    echo '<script>window.location = "view.php";</script>';
