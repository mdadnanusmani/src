...
<div id="jsstadmin-wrapper">
    ...
    <span class="js-adminhead-title"><a class="jsanchor-backlink" href="<?php echo admin_url('admin.php?page=jssupportticket&jstlay=controlpanel');?>"><img alt="image" src="<?php echo jssupportticket::$_pluginpath; ?>includes/images/back-icon.png" /></a><span class="jsheadtext"><?php echo __('Department', 'js-support-ticket') ?></span>
        <a class="js-add-link button" href="?page=department&jstlay=adddepartment"><img alt="image" src="<?php echo jssupportticket::$_pluginpath; ?>includes/images/add_icon.png" /><?php echo __('Add Department', 'js-support-ticket') ?></a>
        </span>
        ...
        <?php if (!empty(jssupportticket::$_data[0])) { ?>
            <div class="js-filter-form-list">
                ...
                echo " : "; ?></span><?php echo $department->outgoingemail; ?></div>
                        ...
                        echo " : "; ?></span>
                        <a href="<?php echo wp_nonce_url('?page=department&task=changedefault&action=jstask&departmentid='. $department->id.'&default='.$department->isdefault, 'change-default');?>"> <img alt="image" src="<?php echo jssupportticket::$_pluginpath  .'includes/images/' . $default; ?>"/> </a></div>
                ...
                echo " : "; ?></span>
                            <?php if ($number != 0 || $pagenum > 1) {
                                $url  = "?page=department&task=ordering&action=jstask&order=up&departmentid=".$department->id;
                                if($pagenum > 1){
                                    $url .= '&pagenum=' . $pagenum;
                                } ?>
                            ...
                            if ($number < $count) { ?>
                                <?php if ($number != 0 || $pagenum > 1) {
                                    $url  = "?page=department&task=ordering&action=jstask&order=down&departmentid=".$department->id;
                                    if($pagenum > 1){
                                        $url .= '&pagenum=' . $pagenum;
                                    } ?>
                        ...
                        <div class="js-col-xs-12 js-col-md-1 fourth js-textaligncenter"><span class="js-filter-form-data-xs"><?php echo __('Status', 'js-support-ticket');
                        echo " : "; ?></span><a href="<?php echo wp_nonce_url('?page=department&task=changestatus&action=jstask&departmentid='.$department->id,'change-status');?>"> <img alt="image" src="<?php echo jssupportticket::$_pluginpath . 'includes/images/' . $status; ?>"/> </a></div>
                ...
                echo " : "; ?></span><?php echo date_i18n(jssupportticket::$_config['date_format'], strtotime($department->created)); ?></div>
                        <div class="js-col-xs-12 js-col-md-2 sixth js-filter-form-action-hl-xs">
                            <a href="?page=department&jstlay=adddepartment&jssupportticketid=<?php echo $department->id; ?>"><img alt="image" src="<?php echo jssupportticket::$_pluginpath; ?>includes/images/edit.png" /></a>&nbsp;&nbsp;
                            <a onclick="return confirm('<?php echo __('Are you sure to delete', 'js-support-ticket'); ?>');" href="<?php echo wp_nonce_url('?page=department&task=deletedepartment&action=jstask&departmentid='.$department->id,'delete-department');?>"><img alt="image" src="<?php echo jssupportticket::$_pluginpath; ?>includes/images/remove.png" /></a></div>
                    </div>
                <?php
                $number++;
        }
