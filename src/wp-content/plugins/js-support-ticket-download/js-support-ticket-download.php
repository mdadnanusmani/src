<?php
/**
* @package JS Support Ticket Downloads
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Downloads
Plugin URI: https://www.joomsky.com
Description:  Downloads Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTDownload{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-download-version',JSSTDownload::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('download',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-download-version',JSSTDownload::$_addon_version);
        include_once 'includes/activation.php';
        JSTDownloadactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('download');
        }
        include_once 'includes/activation.php';
        JSTDownloadactivation::jssupportticketaddon_deactivate();
    }

}

$JSSTDownload = new JSSTDownload();
