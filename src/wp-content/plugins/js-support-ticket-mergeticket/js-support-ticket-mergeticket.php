<?php
/**
* @package JS Support Ticket Mergeticket
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Merge Ticket
Plugin URI: https://www.joomsky.com
Description:  Merge Ticket Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTMergeticket{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-mergeticket-version',JSSTMergeticket::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('mergeticket',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-mergeticket-version',JSSTMergeticket::$_addon_version);
        // include_once 'includes/activation.php';
        // JSTMergeticketeactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('mergeticket');
        }
        // include_once 'includes/activation.php';
        // JSTMergeticketeactivation::jssupportticketaddon_deactivate();
    }
}

$JSSTMergeticket = new JSSTMergeticket();
