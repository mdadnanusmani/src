...
class JSSTknowledgebaseController {
    ...
    static function savecategory() {
        ...
        if (is_admin()) {
            ...
            $url = jssupportticket::makeUrl(array('jstmod'=>'knowledgebase', 'jstlay'=>'stafflistcategories'));
        }
        wp_redirect($url);
        exit;
    }
    ...
    static function changestatuscategory() {
        $nonce = JSSTrequest::getVar('_wpnonce');
        if (! wp_verify_nonce( $nonce, 'change-status-category') ) {
            die( 'Security check Failed' );
        }
        $id = JSSTrequest::getVar('categoryid');
    ...
    static function deletearticle() {
        $nonce = JSSTrequest::getVar('_wpnonce');
        if (! wp_verify_nonce( $nonce, 'delete-article') ) {
            die( 'Security check Failed' );
        }
    ...
    static function changestatusarticle() {
        $nonce = JSSTrequest::getVar('_wpnonce');
        if (! wp_verify_nonce( $nonce, 'change-status-article') ) {
            die( 'Security check Failed' );
        }
        $id = JSSTrequest::getVar('articleid');
