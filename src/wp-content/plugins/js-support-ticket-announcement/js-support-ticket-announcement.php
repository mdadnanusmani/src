<?php
/**
* @package JS Support Ticket Announcement
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Announcement
Plugin URI: https://www.joomsky.com
Description:  Announcements Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTAnnouncement{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-announcement-version',JSSTAnnouncement::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('announcement',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }

        update_option('jsst-addon-announcement-version',JSSTAnnouncement::$_addon_version);
        include_once 'includes/activation.php';
        JSTAnnouncementactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('announcement');
        }
        include_once 'includes/activation.php';
        JSTAnnouncementactivation::jssupportticketaddon_deactivate();
    }

}
$JSSTAnnouncement = new JSSTAnnouncement();
?>