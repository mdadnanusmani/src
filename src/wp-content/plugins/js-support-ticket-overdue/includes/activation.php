...
class JSTOverdueeactivation {

    static function jssupportticketaddon_activate() {
        // Install Database
        JSTOverdueeactivation::runSQL();
    }

    static function jssupportticketaddon_deactivate() {
        // string config for reactivation
        $query = "SELECT configvalue,configname  FROM `" . jssupportticket::$_db->prefix . "js_ticket_config` WHERE addon = 'overdue';";
        ...
        update_option('JSToverdueactivation_config_string',json_encode($config_array));// config array stored in option to be used later.
    }
    ...
    static private function runSQL() {
        ...
        $config_string = get_option('JSToverdueactivation_config_string');
        if($config_string != '' && !empty(json_decode($config_string,true))){ // if there are stored configuraations in the option table
            ...
            $config_array = array('ticket_overdue' => '5', 'ticket_mark_overdue_admin' => '1', 'ticket_mark_overdue_user' => '1', 'ticket_overdue_type' => '1' );
        }
