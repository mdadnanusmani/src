...
class JSTFeedbackactivation {

    static function jssupportticketaddon_activate() {
        // Install Database
        JSTFeedbackactivation::runSQL();
    }

    static function jssupportticketaddon_deactivate() {

        // string config for reactivation
        $query = "SELECT configvalue,configname  FROM `" . jssupportticket::$_db->prefix . "js_ticket_config` WHERE addon = 'feedback';";
        ...
        update_option('JSTFeedbackactivation_config_string',json_encode($config_array));// config array stored in option to be used later.
    ...
    static private function runSQL() {
        ...
        $config_string = get_option('JSTFeedbackactivation_config_string');
        if($config_string != '' && !empty(json_decode($config_string,true))){ // if there are stored configuraations in the option table
            ...
            $config_array = array('cplink_feedback_staff' => '1', 'feedback_email_delay_type' => '1', 'feedback_email_delay' =>'30', 'ticket_feedback_user' => '1');
        }
