<?php
/**
* @package JS Support Ticket User Options
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket User Options
Plugin URI: https://www.joomsky.com
Description: User options addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTUseroptions{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-useroptions-version',JSSTUseroptions::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('useroptions',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-useroptions-version',JSSTUseroptions::$_addon_version);
        include_once 'includes/activation.php';
        JSTUseroptionsactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('useroptions');
        }
        include_once 'includes/activation.php';
        JSTUseroptionsactivation::jssupportticketaddon_deactivate();
    }

}

$JSSTUseroptions = new JSSTUseroptions();
