<?php
/**
* @package JS Support Ticket Faq
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Faq
Plugin URI: https://www.joomsky.com
Description:  FAQ for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTFaq{
    ...
    function jsst_addon_update_check() {
		...
		if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
		    ...
		    update_option('jsst-addon-faq-version',JSSTFaq::$_addon_version);
		}
	}
	...
	function jstsupportitkcetaddon_activate_hook(){
		...
		if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('faq',$plugin_version)){
		    do_action('jsst_addon_update_date_failed');
		}
		update_option('jsst-addon-faq-version',JSSTFaq::$_addon_version);
		include_once 'includes/activation.php';
		JSTFaqactivation::jssupportticketaddon_activate();
	...
	function jssupportticketaddon_deactivate_hook(){
		if (class_exists('jssupportticket') ) {
			JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('faq');
        }
		include_once 'includes/activation.php';
		JSTFaqactivation::jssupportticketaddon_deactivate();
    }
