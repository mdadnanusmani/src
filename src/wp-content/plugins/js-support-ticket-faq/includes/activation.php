...
class JSTFaqactivation {

    static function jssupportticketaddon_activate() {
        // Install Database
        JSTFaqactivation::runSQL();
    }

    static function jssupportticketaddon_deactivate() {
        // string config for reactivation
        $query = "SELECT configvalue,configname  FROM `" . jssupportticket::$_db->prefix . "js_ticket_config` WHERE addon = 'faq';";
        ...
        update_option('JSTFaqactivation_config_string',json_encode($config_array));// config array stored in option to be used later.
    }
    ...
    static private function runSQL() {
        ...
        $config_string = get_option('JSTFaqactivation_config_string');
