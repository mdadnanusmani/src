<?php
/**
* @package JS Support Ticket Auto Close
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Auto Close
Plugin URI: https://www.joomsky.com
Description:  Auto Close Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTAutoClose{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-autoclose-version',JSSTAutoClose::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('autoclose',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-autoclose-version',JSSTAutoClose::$_addon_version);
        include_once 'includes/activation.php';
        JSTAutoCloseactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('autoclose');
        }
        include_once 'includes/activation.php';
        JSTAutoCloseactivation::jssupportticketaddon_deactivate();
    }

}

$JSSTAutoClose = new JSSTAutoClose();
