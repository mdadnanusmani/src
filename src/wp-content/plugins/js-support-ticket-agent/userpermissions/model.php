...
class JSSTuserpermissionsModel {
    ...
    function assignRolePermissionsToUser($agent) {
        //delete any other acl user permissions which is not the current role id
        $query = "DELETE FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_permissions` WHERE uid = " . $agent->uid;
        jssupportticket::$_db->query($query);
        $query = "INSERT INTO `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_permissions` (uid, roleid, staffid, permissionid, `isgranted`, status)
                SELECT '" . $agent->uid . "' AS uid,'" . $agent->roleid . "' AS roleid,'" . $agent->id . "' AS staffid, permissionid, `isgranted`, status FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_role_permissions` WHERE roleid = " . $agent->roleid;
        jssupportticket::$_db->query($query);
        //delete any other acl user permissions which is not the current role id
        $query = "DELETE FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_access_departments` WHERE uid = " . $agent->uid;
        jssupportticket::$_db->query($query);
        $query = "INSERT INTO `" . jssupportticket::$_db->prefix . "js_ticket_acl_user_access_departments` (uid, roleid, staffid, departmentid, status)
                SELECT '" . $agent->uid . "' AS uid,'" . $agent->roleid . "' AS roleid,'" . $agent->id . "' AS staffid, departmentid, status FROM `" . jssupportticket::$_db->prefix . "js_ticket_acl_role_access_departments` WHERE roleid = " . $agent->roleid;
        jssupportticket::$_db->query($query);
        /*
    ...
    function storeUserPermissions($data) {
        ...
        if (!$roleid)
            $roleid = JSSTincluder::getJSModel('agent')->getRoleidByStaffId($staffid);
        $new_permissions = array();
        ...
        foreach ($new_permissions AS $perid) {
            ...
            if ($insert) {
                ...
                $row = JSSTincluder::getJSTable('acl_user_permissions');
