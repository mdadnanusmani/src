...
class JSSTroleModel {
    ...
    function getRoles() {
        ...
        $name = addslashes(trim(JSSTrequest::getVar($rolename)));
        ...
        if ($formsearch == 'JSST_SEARCH') {
            ...
            if(isset($_SESSION['JSST_SEARCH'])){
               unset($_SESSION['JSST_SEARCH']['rolename']);
            }
        }
    ...
    function storeRole($data) {
        ...
        if ($data['id'] == '') { //new
            ...
            if ($isexist == 2) {
                JSSTmessage::setMessage(__('Already exists', 'js-support-ticket'), 'error');
                return;
            }
        }
        ...
        if ($error == 0) {
            $last_role_id = $row->id;
            if(!isset($data['roledepdata'])){ // changed this bcz it was causing probelm in case of all department unset
                $data['roledepdata'] = array();
            }
            JSSTincluder::getJSModel('roleaccessdepartments')->storeRoleAccessDepartments($data['roledepdata'], $last_role_id);
            if(!isset($data['roleperdata'])){ // changed this bcz it was causing probelm in case of all permissoins unset
                $data['roleperdata'] = array();
            }
    ...
    function removeRole($id) {
        ...
        if ($this->CanRemoveRole($id) == true) {
            ...
            JSSTmessage::setMessage(__('Role in use cannot deleted', 'js-support-ticket'), 'error');
            return;
        }
    }
