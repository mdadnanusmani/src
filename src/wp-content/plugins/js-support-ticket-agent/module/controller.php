...
class JSSTagentController {
    ...
    function handleRequest() {
        ...
        if (self::canaddfile()) {
            switch ($layout) {
                ...
                case 'staffs':
                    ...
                    if (jssupportticket::$_data['permission_granted']) {
                        JSSTincluder::getJSModel('agent')->getStaffs();
                    }
                    break;
                case 'admin_addstaff':
                case 'addstaff':
                    ...
                    if (jssupportticket::$_data['permission_granted']) {
                        JSSTincluder::getJSModel('agent')->getStaffForForm($id);
                    }
                    break;
                ...
                case 'staffpermissions':
                    ...
                    if (jssupportticket::$_data['permission_granted']) {
                        ...
                        JSSTincluder::getJSModel('agent')->getPermissionsForForm($id);
                    }
                    break;
                case 'myprofile':
                    JSSTincluder::getJSModel('agent')->getMyProfile();
                    break;
            ...
            $module = JSSTrequest::getVar($module, null, 'agent');
    ...
    static function savestaff() {
        ...
        if (is_admin()) {
            $url = admin_url("admin.php?page=agent&jstlay=staffs");
        } else {
            $url = jssupportticket::makeUrl(array('jstmod'=>'agent','jstlay'=>'staffs'));
        }
        wp_redirect($url);
        exit;
    }
    ...
    static function deletestaff() {
        ...
        if (! wp_verify_nonce( $nonce, 'delete-staff') ) {
            die( 'Security check Failed' );
        }
        $id = JSSTrequest::getVar('staffid');
        JSSTincluder::getJSModel('agent')->removeStaff($id);
        if (is_admin()) {
            $url = admin_url("admin.php?page=agent&jstlay=staffs");
        } else {
            $url = jssupportticket::makeUrl(array('jstmod'=>'agent','jstlay'=>'staffs'));
    ...
    static function changestatus() {
        ...
        $id = JSSTrequest::getVar('staffid');
        JSSTincluder::getJSModel('agent')->changeStatus($id);
        $url = admin_url("admin.php?page=agent&jstlay=staffs");
        wp_redirect($url);
        exit;
    }
...
$agentController = new JSSTagentController();
?>
