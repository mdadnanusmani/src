<?php
/**
* @package JS Support Ticket Desktop Notification
* @version 1.0.1
*/
/*
Plugin Name: JS Support Ticket Desktop Notification
Plugin URI: https://www.joomsky.com
Description:  Desktop Notifications for JS Support Ticket.
Author: Joom Sky
Version: 1.0.1
...
define('TICKET_NOTIFY_MESSAGE_JS', plugins_url( 'includes/js/firebase-messaging-sw.js', __FILE__ ));
define('JSTN', 'jssupportticketnotification');
// require_once 'includes/updater/jsstupdater.php';

//class jsticketdesktopnotification extends JS_Updater{
class JSSTNotification{
    ...
    function __construct() {
        ...
        add_filter('cron_schedules', array($this,'jsst_weekly_cron_notify' ));
        add_action('send_push_notification',array($this , 'sendpushnotification'));
        add_action('ticket_notify_preferences',array($this , 'ticketuserpreferences'));
        add_action('jsstnotification_cronjobs_action', array($this,'jsstnotification_cronjobs'));
        add_action('wp_head', array($this,'registerscript'));
    }
    ...
    function jsst_addon_update_check() {
		...
		if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
		    ...
		    update_option('jsst-addon-notification-version',JSSTNotification::$_addon_version);
		}
	}
	...
	if ( !class_exists('jssupportticket') ) {
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('notification',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
		update_option('jsst-addon-notification-version',JSSTNotification::$_addon_version);
		include_once 'includes/activation.php';
		JSTNotificationactivation::jssupportticketaddon_activate();
		wp_schedule_event(time(), 'weekly', 'jsstnotification_cronjobs_action');
        update_option('jsstnotification_do_activation_redirect', true);
        $notificationkey = get_option('transaction_key_for_js-support-ticket-notification');
        $this->updateNotificationKeyToken($notificationkey);
        // wp_schedule_event(time(), 'weekly', 'jsstnotification_cronjobs_action');
        // update_option('jsstnotification_do_activation_redirect', true);
    }
	...
	function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('notification');
        }
        include_once 'includes/activation.php';
		JSTNotificationactivation::jssupportticketaddon_deactivate();
    }
    ...
    function updateNotificationKeyToken($key){
        if($key != ""){
            $query = "UPDATE `" . jssupportticket::$_db->prefix . "js_ticket_config` SET `configvalue`= '" . $key . "' WHERE configname='". md5(JSTN) ."'";
            jssupportticket::$_db->query($query);
        }
    }
}

$JSSTNotification = new JSSTNotification();
