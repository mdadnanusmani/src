<?php
/**
* @package JS Support Ticket Private Credentials
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Private Credentials
Plugin URI: https://www.joomsky.com
Description:  Private Credentials Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTPrivatecredentialsaddon{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-privatecredentials-version',JSSTPrivatecredentialsaddon::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('privatecredentials',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-privatecredentials-version',JSSTPrivatecredentialsaddon::$_addon_version);
        include_once 'includes/activation.php';
        JSTPrivatecredentialsactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        include_once 'includes/activation.php';
        JSTPrivatecredentialsactivation::jssupportticketaddon_deactivate();
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('privatecredentials');
        }
    }
}

$JSSTPrivatecredentialsaddon = new JSSTPrivatecredentialsaddon();
?>