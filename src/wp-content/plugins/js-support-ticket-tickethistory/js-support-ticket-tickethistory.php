<?php
/**
* @package JS Support Ticket Ticket History
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Ticket History
Plugin URI: https://www.joomsky.com
Description:  Ticlet History Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTTicketHistory{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-activitylog-version',JSSTTicketHistory::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('tickethistory',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-activitylog-version',JSSTTicketHistory::$_addon_version);
        include_once 'includes/activation.php';
        JSTActivityLogeactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('tickethistory');
        }
        include_once 'includes/activation.php';
        JSTActivityLogeactivation::jssupportticketaddon_deactivate();
    }

}

$JSSTTicketHistory = new JSSTTicketHistory();
