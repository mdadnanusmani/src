<?php
/**
* @package JS Support Ticket Max Ticket
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Max Ticket
Plugin URI: https://www.joomsky.com
Description:  Max Ticket Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTMaxTicket{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-maxticket-version',JSSTMaxTicket::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('maxticket',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-maxticket-version',JSSTMaxTicket::$_addon_version);
        include_once 'includes/activation.php';
        JSTMaxTicketactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('maxticket');
        }
        include_once 'includes/activation.php';
        JSTMaxTicketactivation::jssupportticketaddon_deactivate();
    }

}

$JSSTMaxTicket = new JSSTMaxTicket();
