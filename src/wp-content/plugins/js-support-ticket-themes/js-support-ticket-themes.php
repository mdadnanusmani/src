<?php
/**
* @package JS Support Ticket Themes
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Themes
Plugin URI: https://www.joomsky.com
Description:  Themes Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTThemes{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-themes-version',JSSTThemes::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('themes',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-themes-version',JSSTThemes::$_addon_version);
        // include_once 'includes/activation.php';
        // JSTThemeseactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('themes');
        }
        // include_once 'includes/activation.php';
        // JSTThemeseactivation::jssupportticketaddon_deactivate();
    }
}

$JSSTThemes = new JSSTThemes();
?>