<?php
/**
* @package JS Support Ticket Dashboard Widgets
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Dashboard Widgets
Plugin URI: https://www.joomsky.com
Description:  Dashboard Widgets Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTDashboardwidgets{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-dashbaordwidgets-version',JSSTDashboardwidgets::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('dashboardwidgets',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-dashbaordwidgets-version',JSSTDashboardwidgets::$_addon_version);
        include_once 'includes/activation.php';
        JSTDashboardwidgetsactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('dashboardwidgets');
        }
        include_once 'includes/activation.php';
        JSTDashboardwidgetsactivation::jssupportticketaddon_deactivate();
    }

}

$JSSTDashboardwidgets = new JSSTDashboardwidgets();
