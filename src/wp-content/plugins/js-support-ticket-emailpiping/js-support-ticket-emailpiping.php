<?php
/**
* @package JS Support Ticket Email piping
* @version 1.0.1
*/
/*
Plugin Name: JS Support Ticket Email piping
Plugin URI: https://www.joomsky.com
Description:  Email piping Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.1
...
class JSSTemailpiping{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-ticketviaemail-version',JSSTemailpiping::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('emailpiping',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-ticketviaemail-version',JSSTemailpiping::$_addon_version);
        include_once 'includes/activation.php';
        JSTTicketviaemaileactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('emailpiping');
        }
        include_once 'includes/activation.php';
        JSTTicketviaemaileactivation::jssupportticketaddon_deactivate();
    }


}

$JSSTemailpiping = new JSSTemailpiping();
