<?php
/**
* @package JS Support Ticket BanEmail
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket BanEmail
Plugin URI: https://www.joomsky.com
Description:  Ban Email Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTBanEmail{
	...
	function jsst_addon_update_check() {
		...
		if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
		    ...
		    update_option('jsst-addon-banemail-version',JSSTBanEmail::$_addon_version);
		}
	}
	...
	if ( !class_exists('jssupportticket') ) {
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('banemail',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
		update_option('jsst-addon-banemail-version',JSSTBanEmail::$_addon_version);
		include_once 'includes/activation.php';
		JSTBanEmailactivation::jssupportticketaddon_activate();
	...
	function jssupportticketaddon_deactivate_hook(){
		if (class_exists('jssupportticket') ) {
			JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('banemail');
        }
		include_once 'includes/activation.php';
		JSTBanEmailactivation::jssupportticketaddon_deactivate();
    }
