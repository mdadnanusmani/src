...
class JSTBanEmailactivation {

    static function jssupportticketaddon_activate() {
        // Install Database
        JSTBanEmailactivation::runSQL();
    }

    static function jssupportticketaddon_deactivate() {
        // string config for reactivation
        $query = "SELECT configvalue,configname  FROM `" . jssupportticket::$_db->prefix . "js_ticket_config` WHERE addon = 'banemail';";
        ...
        update_option('JSTBanEmailactivation_config_string',json_encode($config_array));// config array stored in option to be used later.
    }
    ...
    static private function runSQL() {
        ...
        $config_string = get_option('JSTBanEmailactivation_config_string');
        if($config_string != '' && !empty(json_decode($config_string,true))){ // if there are stored configuraations in the option table
            ...
            $config_array = array('banemail_mail_to_admin' => '1', 'ticket_ban_email_admin' => '1', 'ticket_ban_email_staff' => '1', 'ticket_ban_email_user' => '1', 'ticker_ban_eamil_and_close_ticktet_admin' => '1', 'ticker_ban_eamil_and_close_ticktet_staff' => '1', 'ticker_ban_eamil_and_close_ticktet_user' => '1', 'unban_email_admin' => '1', 'unban_email_staff' => '1', 'unban_email_user' => '1');
        }
