<?php
/**
* @package JS Support Ticket Canned Responses
* @version 1.0.0
*/
/*
Plugin Name: JS Support Ticket Canned Responses
Plugin URI: https://www.joomsky.com
Description:  Canned Responses Addon for JS Support Ticket.
Author: Joom Sky
Version: 1.0.0
...
class JSSTCannedresponses{
    ...
    function jsst_addon_update_check() {
        ...
        if(file_exists(dirname(__FILE__).'/includes/addon-sql.php') && realpath(dirname(__FILE__).'/includes/addon-sql.php')){
            ...
            update_option('jsst-addon-premaderesponses-version',JSSTCannedresponses::$_addon_version);
        }
    }
    ...
    function jstsupportitkcetaddon_activate_hook(){
        ...
        if(!JSSTincluder::getJSModel('jssupportticket')->updateDate('cannedresponses',$plugin_version)){
            do_action('jsst_addon_update_date_failed');
        }
        update_option('jsst-addon-premaderesponses-version',JSSTCannedresponses::$_addon_version);
        include_once 'includes/activation.php';
        JSTPremaderesponseseactivation::jssupportticketaddon_activate();
    ...
    function jssupportticketaddon_deactivate_hook(){
        if (class_exists('jssupportticket') ) {
            JSSTincluder::getJSModel('premiumplugin')->logAddonDeactivation('cannedresponses');
        }
        include_once 'includes/activation.php';
        JSTPremaderesponseseactivation::jssupportticketaddon_deactivate();
    }

}

$JSSTCannedresponses = new JSSTCannedresponses();
